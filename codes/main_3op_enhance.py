...
##############################################################
            ...
            if attn_map_out:
                ...
                # # 根据 t, second_order, id, type 构造文件名
                # filename = f"{t}_{second_order}_{id}_{type}_inv{inverse}_attn_map.pt"
                # file_path = os.path.join(folder_path, filename)

                # # 保存 attn_map 到文件（使用 torch.save 保存张量）
                # torch.save(attn_map, file_path)
                # print("Attention Map saved to:", file_path)
            ...
            def enhanced_scaled_dot_product_attention(query, key, value, alpha=2.0, v=1.5, i_list=None, K=1):
                ...
                with torch.no_grad():  # 禁用梯度计算以提升性能
                    ...
                    # ==================== 语义增强处理 ====================
                    image_token_start = 512  # 假设图像token从第512个位置开始
                    text_token_end = 512     # 文本token结束位置（CLIP等模型常见设置）
...
def main(args):
    ...
    if args.eval_datasets == '':
        ...
        dataloader = [img, args.source_prompt, args.target_prompt, args.enhanced_list]
    else:
    ...
    ilist_json_path = args.ilist_json_path
    with open(ilist_json_path, 'r') as f:
    ...
    for img_float32, source_prompt, target_prompt in dataloader:
        ...
        print(f"当前count: {count}, 使用ilist: {current_ilist}")    
...
if __name__ == "__main__":
    ...
    parser.add_argument('--enhanced_list', type=int, nargs='+', default=[],
                        help='指定需要增强的层或步骤列表（用空格分隔多个数字）')
    parser.add_argument('--ilist_json_path', type=str, default='',
                        help='the path of ilist json file')    
