...
class RfSolverFluxAttnProcessor2_0_3opt:
    ...
    def __call__(
            ...
            K: Optional[int] = 5,
            alpha: Optional[float] = 3,
            v: Optional[float] = 3,
            SVD_start: Optional[float] = 0.96,
            SVD_end: Optional[float] = 0.85,
                    ...
                    if stable_flow: 
                        ...
                        if inject and id in [
                            ...
                            if inverse:
                                ...
                                query[:, :, 1024:, :] =feature[feature_name].cuda()
                        ...
                        if t>=0.97:
                            ...
                            if inject and ( id in [
                                ...
                                if inverse:
                                    ...
                                    value[:, :, 1024:, :] = feature[feature_name].cuda()
##############################################################
...
def main(args):
    ...
    joint_attention_kwargs['K'] = args.K
    joint_attention_kwargs['v'] = args.v
    joint_attention_kwargs['alpha'] = args.alpha
    joint_attention_kwargs['SVD_start'] = args.SVD_start
    joint_attention_kwargs['SVD_end'] = args.SVD_end
    ...
    for img_float32, source_prompt, target_prompt, current_ilist in dataloader:
        # 处理增强列表
        if args.eval_datasets =='':
        #     current_ilist = ilist_entries[count]['ilist']
        # else:
            current_ilist = args.enhanced_list  # 直接使用命令行参数

        joint_attention_kwargs['enhanced_list'] = current_ilist
        print("current_ilist", current_ilist)
        ...
        print('######### until Evaluation Results ###########')
        until_clip_score = mean_clip_score / count
        print(f"==> clip-T score: {until_clip_score:.4f}")
        until_clip_v_score = mean_clip_v_score / count
        print(f"==> clip-I score: {until_clip_v_score:.4f}")    
        until_mse_score = mean_mse_score / count
        print(f"==> mse score: {until_mse_score:.4f}")
        until_psnr_score = mean_psnr_score / count
        print(f"==> psnr score: {until_psnr_score:.4f}")
        until_lpips_score = mean_lpips_score / count
        print(f"==> lpips score: {until_lpips_score:.4f}")
        until_ssim_score = mean_ssim_score / count
        print(f"==> ssim score: {until_ssim_score:.4f}")
    ...
    del out, img_latent, img_latents, inversed_latent_s,inversed_latent_t, pipe, img, joint_attention_kwargs
