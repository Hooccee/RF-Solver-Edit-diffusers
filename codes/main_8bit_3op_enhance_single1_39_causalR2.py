...
#*******************************************************************************
        ...
        joint_attention_kwargs_var=copy.deepcopy(joint_attention_kwargs)
        ...
        for i in range(len(target_var_list)):
            ...
            target_prompt_var =target_var_list[i]
            print("target_prompt_var", target_prompt_var)
            if args.use_inversed_latents:
                inversed_latent_t ,joint_attention_kwargs_var = interpolated_inversion(
                    pipe, 
                    img_latent_nudge,
                    DTYPE=DTYPE, 
                    num_steps=args.num_steps, 
                    ...
                    clip_prompt=source_prompt,
                    guidance_scale = 1,
                    joint_attention_kwargs=joint_attention_kwargs_var)    
            else:
                inversed_latent_t = None
            for i in joint_attention_kwargs['feature']: 
                joint_attention_kwargs['feature'][i]=joint_attention_kwargs['feature'][i]+(joint_attention_kwargs_var['feature'][i]/(args.target_var_num+1))
