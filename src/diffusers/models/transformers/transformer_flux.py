...
@maybe_allow_in_graph
class RfSolverFluxSingleTransformerBlock(FluxSingleTransformerBlock):
    def __init__(self, dim, num_attention_heads, attention_head_dim, mlp_ratio=4.0):
        super().__init__(dim, num_attention_heads, attention_head_dim, mlp_ratio)
        processor = RfSolverFluxAttnProcessor2_0()
        self.attn = RfSolverAttention(
            query_dim=dim,
            cross_attention_dim=None,
            dim_head=attention_head_dim,
            heads=num_attention_heads,
...
@maybe_allow_in_graph
class RfSolverFluxTransformerBlock(FluxTransformerBlock):
...
class RfSolverFluxTransformer2DModel(ModelMixin, 
    ConfigMixin, 
    PeftAdapterMixin, 
    ...
    _supports_gradient_checkpointing = True
    _no_split_modules = ["RfSolverFluxTransformerBlock", "RfSolverFluxSingleTransformerBlock"]
    ...
    def __init__(
        ...
        self.single_transformer_blocks = nn.ModuleList(
            [
                RfSolverFluxSingleTransformerBlock(
                    dim=self.inner_dim,
                    num_attention_heads=self.config.num_attention_heads,
                    attention_head_dim=self.config.attention_head_dim,
                )
    ...
    def forward(
        ...
        return Transformer2DModelOutput(sample=output),joint_attention_kwargs